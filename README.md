...
The device restart behavior should be "Determine behavior based on return codes."  If the default user registry hive is edited and cannot be dismounted then exit code 3010 will be returned, indicating the device should reset at the end of device setup.
